...
function createListRow( marker )
{
    ...
    var onclick = function() { alert( marker.address ); return false };
...
{
    changeLinksColour( this.links, "blue" )
    changeMarkerColour( this, "blue" );
}

function activateMarker()
{
    changeLinksColour( this.links, "red" )
    changeMarkerColour( this, "red" );
}
...
function createMarker( school ) 
{
    ...
    marker = new GxMarker( point, icon, map );
    for ( var j = 0; j < school.attributes.length; j++ )
    {
        var attr = school.attributes.item( j );
        if ( attr.value ) marker[attr.name] = attr.value;
    }
    marker.letter = getLetter( marker.ofsted_type );
    icon.image = icon_root_url + 'marker-BLUE-' + marker.letter + '.png';
    marker.onMouseOver = activateMarker;
    marker.onMouseOut = deActivateMarker;
    marker.showInfo = showInfo;
